package com.org.zapbook.genericCode;
...
import com.eva.vtiger.listner.CaptureScriptLogs;
...
public class WebDriverUtil {
	...
	public void getProperties() {
		...
		InputStream in = null;
		try {
			in = new FileInputStream("confiq.properties");
			prop.load(in);
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 * 
	 * @param browserName this parameter used to launch browser which browser you
	 *                    want to give name
	 */
	public void launchBrower(String browserName) {
		if (browserName.equalsIgnoreCase("chrome")) {
			...
			dataMap.put("download.default_directory", System.getProperty("user.dir") + "\\ImportData");
			chroptions.setExperimentalOption("prefs", dataMap);
			...
			System.out.println(cap.getPlatformName().getCurrent());
		...
		WebDriverListener webListener = new CaptureScriptLogs();
		driver = new EventFiringDecorator<WebDriver>(webListener).decorate(driver);
	}
	 ...
	 * 
	 * @param fileName this is parameter used to give the filename where you want to
	...
	public ExtentTest SnapShot(String fileName) {
		...
		return extt.addScreenCaptureFromPath(file.getAbsolutePath());
	}
	...
	/**
	 * @param extReport1   we used this parameter to call createTest if we will use
	 *                     this method then it will take ExtentReports reference
	 *                     variable as parameter then we will have to pass
	 * @param testCaseName this parameter used to name of testCase id we have to
	 *                     pass as string type argument
	 * 
	 */
	...
	/**
	 * this method is used flush all our report we can use flush end of the
	 * programing report
	 * 
	 * @param this parameter is used to call flush method with ExtentReports
	 *             reference variable
	 */
	public void flushed() {
		extReport.flush();
	...
	/**
	 * here we used this method to generate our programing report to give the
	 * message
	 * 
	...
	/**
	 * this method is used to generate date which is used in our report to identify
	 * different report
	 * 
	 * @return this method returns timestamp which is string type and returns curent
	 *         generated date
	 */
	 ...
	 * 
	 * @param locatorValue this parameter is used to give the locator value of any
	 *                     element on UI
	 * @param locatorType  this parameter is used to give the type of locator value
	 *                     of any element on UI
	...
	/**
	 * @discription:this method is use to search the multiple elements on UI By
	 *                   locator
	 * @param locatorValue this parameter is used to give the locator value of any
	 *                     element on UI
	 * @param locatorType  this parameter is used to give the type of locator of
	 *                     locator value of any element on UI
	...
	/**
	 * @discription:this method is used to check element to verify whether element
	 *                   is actionable or not
	 * @param we          this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
	 * @param elementName this parameter is used to we have to give the elementName
	 *                    Which we are working on element that time
	...
	/**
	 * @discription:this method is used to select the element option from dropdown
	 *                   by using selectByValueAttribute method of select class
	 * @param we                     this parameter is used to search the element by
	 *                               locatorType and locatorValue on UI which type
	 *                               are webElement
	 * @param elementName            this parameter is used to we have to give the
	 *                               elementName which we are working on element
	 *                               that time
	 * @param valueAttributeToSelect this parameter is used to give the attribute
	 *                               value of any attribute type
	 */
...
//			WebElement we = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			extt.log(Status.INFO, MarkupHelper.createLabel(valueAttributeToSelect + "isn't performed successfully",
					ExtentColor.LIME));
			SnapShot("TakeScreenShotFromSelectByValueAttributeMethod");
			e.printStackTrace();
	...
	/**
	 * @discription:this method is used to select the element option from dropdown
	 *                   by using SelectByIndex method of select class
	 * @param locatorValue       this parameter is used to search the element on UI
	 *                           by locatorValue which type are webElement
	 * @param locatorType        this parameter is used to search the element on UI
	 *                           by locatorType which type are webElement
	 * @param elementName        this parameter is used to we have to give
	 *                           elementName which we are working on element that
	 *                           time
	 * @param indexValueToSelect this parameter is used to give the indexing which
	 *                           element we want to select element option from
	 *                           dropDown
	 */
...
//			WebElement we = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (checkVisibility == true) {
				...
				extt.log(Status.INFO,
						MarkupHelper.createLabel(elementName + "is performed successfully", ExtentColor.GREY));
			}
	...
	/**
	 * @discription:this method is used to select the element from dropdown by using
	 *                   SelectByVisibleText method of select class
	 * @param we              this parameter is used to search the element by
	 *                        locatorType and locatorValue on UI which type are
	 *                        webElement
	 * @param elementName     this parameter is used to we have to give the
	 *                        elementName which we are working on element that time
	 * @param VisibleToSelect this parameter is used to give the innertext value of
	 *                        any element which is present on dropdown option
	 */
...
//		WebElement web = getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription: this method is used to get first option element which is select
	 *               in dropdown
	 * @param we this parameter is used to search the element by locatorType and
	 *           locatorValue on UI which type are webElement
	 */
...
//			WebElement web = getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription:this method is used to get all options which is present in
	 *                   dropdown
	 * @param we this parameter is used to search the element by locatorType and
	 *           locatorValue on UI which type are webElement
	 */
...
//			WebElement web = getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription:this method is used to fetch innertext value of given element
	 *                   which element have innertext
	 * @param we          this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
	 * @param elementName this parameter is used to we have to give elementName
	 *                    Which are we working on Element that time
...
//			WebElement we = getWebElementLocatorXpath(locatorValue, locatorType);
	 ...
	 *                          webElement
	 * @param elementName       this parameter is used to we have to give
	 *                          elementName Which are we working on Element that
	 *                          time
	 * @param getAttributeValue this parameter is used to give attribute value of
	 *                          any element as a string type
...
//			WebElement we = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (st == true) {
				...
				extt.log(Status.INFO,
						MarkupHelper.createLabel(elementName + "is performed Successfully", ExtentColor.ORANGE));
			}
		} catch (StaleElementReferenceException e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + "isn't performed Successfully", ExtentColor.ORANGE));
			SnapShot("TakeScreenShotMethodOfGetAttrinuteMethodWebElement");
			e.printStackTrace();
...
//			WebElement web = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + "isn't performed successfully", ExtentColor.ORANGE));
			SnapShot("TakeScreenShotMethodOfActionsMouseOver");
			e.printStackTrace();
	...
	/**
	 * @discription:this method is used to drag the element one place to drop
	 *                   another place
	 * @param we1         this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
	 * @param we2         this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
	 * @param elementName this parameter is used to we have to give elementName
	 *                    Which are we working on Element that time
...
//		WebElement we1=getWebElementLocatorXpath(locatorValue1, locatorType1);
			...
			if (checkElement == true) {
				...
				extt.log(Status.INFO, MarkupHelper
						.createLabel(elementName + "DragAndDrop Action Performed Successfully", ExtentColor.BLUE));
			}
		} catch (NoSuchElementException e) {
			extt.log(Status.INFO, MarkupHelper.createLabel(elementName + "DragAndDrop Action Performed UnSuccessfully",
					ExtentColor.BLUE));
			SnapShot(elementName);
			e.printStackTrace();
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (checkElement == true) {
				...
				extt.log(Status.INFO, MarkupHelper
						.createLabel(elementName + "Right Click Action Performed Successfully", ExtentColor.BLUE));

			}
		} catch (NoSuchElementException e) {
			extt.log(Status.INFO, MarkupHelper.createLabel(elementName + "Right click action performed successfully ",
					ExtentColor.BLUE));
			SnapShot("ScreenShot From DragAndDrop");
			e.printStackTrace();
		} catch (Exception e) {
			extt.log(Status.INFO, MarkupHelper.createLabel(elementName + "Right click action performed Unsuccessfully ",
					ExtentColor.BLUE));
			SnapShot("ScreenShot From DragAndDrop");
			e.printStackTrace();
		}
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (checkElement == true) {
				...
				extt.log(Status.INFO, MarkupHelper
						.createLabel(elementName + "double click action   performed successfully ", ExtentColor.BLUE));

			}
		} catch (NoSuchElementException e) {
			extt.log(Status.INFO, MarkupHelper
					.createLabel(elementName + "double click action performed Unsuccessfully ", ExtentColor.BLUE));
			SnapShot("ScreenShot From DragAndDrop");
			e.printStackTrace();
		} catch (Exception e) {
			extt.log(Status.INFO, MarkupHelper
	...
	/**
	 * @discription:this method is used to click on element by using
	 *                   actionsClickMethod
	 * @param we          this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
...
//			WebElement web = getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription:this method is used to send the value in input box by using
	 *                   actionsSendkeysMethod
	 * @param we          this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
...
//			WebElement weFind = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (checkElement == true) {
				...
				extt.log(Status.INFO,
						MarkupHelper.createLabel(elementName + " input performed successfully ", ExtentColor.GREEN));
			}

		} catch (StaleElementReferenceException e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + " input performed unsuccessfully ", ExtentColor.GREEN));
			SnapShot("TakeScreenShotMethodOfAactionsSendkeysMethod");
			e.printStackTrace();
	...
	/**
	 * @discription:this metgod is used to send the value in input box by using
	 *                   WebElementSendKeysMethod
	 * @param we          this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
	 * @param inputValue  this parameter is used to give the value to input box
...
//			WebElement weFind = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			SnapShot("TakeScreenShotMethodOfUsingWebElementSendKeys");
			e.printStackTrace();
...
//		.	WebElement we = getWebElementLocatorXpath(locatorValue, locatorType);
			...
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + " click performed unsuccessfully on ", ExtentColor.RED));
			SnapShot(elementName);
			e.printStackTrace();
	...
	/**
	 * @discription:this method is used to get the title of currnet webPage by using
	 *                   driver.getTitle();
	...
	/**
	 * @discription:this method is used to get the URL of currnet webPage by using
	...
	/**
	 * @dicription: this method is used to handle the frame by indexing using this
	 *              method driver.switchTo().frame(frameIndex);
	 * @param we         this parameter is used to search the element by locatorType
	 *                   and locatorValue on UI which type are webElement
	 * @param frameIndex this parameter is used to give the indexing number by which
	 *                   we can handle frame
	 */
	...
	/**
	 * @dicription: this method is used to swich to parent frame by using this
	 *              method driver.switchTo().parentFrame();
	 */
	 ...
	 * @param frameNameOrIdAtrribute this parameter is used to give name or id
	 *                               attribute value as a String format
	 */
	 ...
	 * @param we                    this parameter is used to search the element on
	 *                              UI by locatorType and locatorValue which type
	 *                              are WebElement
	 * @param frameWebElementMethod this method is used to give the reference
	 *                              variable of WebElement as a string type
	 */
	...
	public void validatePopUpText(String expectedPopText) {
		try {
			String actualText = driver.switchTo().alert().getText();
			if (actualText.equalsIgnoreCase(expectedPopText)) {
				extt.log(Status.PASS,
						"actual Text is " + actualText + " and expectedText is " + expectedPopText + " is matched");
			} else {
				extt.log(Status.PASS,
						"actual Text is " + actualText + " and expectedText is " + expectedPopText + " is not matched");
			}
		} catch (Exception e) {
	...
	/**
	 * @discription: this method is used to if you want to verify message in popup
	 * @param we this parameter is used to search the element by locatorType and
	 *           locatorValue on UI which type are webElement
	 */
	 ...
	 * @param we         this parameter is used to search the element by locatorType
	 *                   and locatorValue on UI which type are webElement
	 * @param inputvalue this parameter is used to send any value in popup as a
	 *                   string type
	 */
...
//			WebElement we=getWebElementLocatorXpath( locatorValue,  locatorType);
	...
	/**
	 * @discription: this method is used to it will wait till the element is
	 *               clickable or not only for that element which element you want
	 * @param we               this parameter is used to search the element by
	 *                         locatorType and locatorValue on UI which type are
	 *                         webElement
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription: this parameter is used to it will wait till the element visible
	 *               or not only for that element which element you want
	 * @param locator
	...
	public void waitForElementVisibility(By locator, int timeOutInSecond) {
		try {
			...
			extt.log(Status.INFO,
					MarkupHelper.createLabel("visibilityOfElementLocated performed successfully", ExtentColor.PINK));
	...
	public void explicityWait(WebElement we, int timeInsecond) {
		if (we.isDisplayed() && we.isEnabled()) {
			...
			extt.log(Status.INFO,
					MarkupHelper.createLabel("visibilityOfElementLocated performed successfully", ExtentColor.ORANGE));
			SnapShot("explicityWait");
	...
	/**
	 * @discription:this parameter is used to it will wait till the element arrives
	 *                   or not only fot that element which element you want
	 * @param locator
	 * @param timeOutInSecond this parameter is used to give the time session
	 */
	...
	/**
	 * @discription: this method is used to it will wait till the element invisible
	 *               or not only for that element which element you want
	 * @param locator
	 * @param atimeOutInSecond this parameter is used to give the time session
	 */
	...
	/**
	 * @discription: this method is used to send any value in input box using
	 *               JavaScriptSendKeys
	 * @param we         this parameter is used to search the element by locatorType
	 *                   and locatorValue on UI which type are webElement
...
//		WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription: this method is used to scroll up and down which element you
	 *               want to scroll
	 * @param we this parameter is used to search the element by locatorType and
	 *           locatorValue on UI which type are webElement
	 */
	...
	/**
	 * @discription:it is used once because it is associated with the driver as long
	 *                 as the driver has used it
	 * @param timeOutInSecond this parameter is used to give the time session
	 */
	...
	/**
	 * @discription: this method is used to verify the actualText and expected text
	 *               to given element
	 * @param we           this parameter is used to search the element by
	 *                     locatorType and locatorValue on UI which type are
	 *                     webElement
	 * @param expectedText this parameter is used to verify the given element to
	 *                     actual element match or not
	 * @param elementName  this parameter is used to we have to give elementName
	 *                     Which are we working on Element that time
	 */
	public void validateText(WebElement we, String expectedText, String elementName) {
		...
		if (actualText.equalsIgnoreCase(expectedText)) {
			extt.log(Status.PASS, elementName + "Text validate Pass. actualText -" + actualText + " && expectedtext -"
					+ expectedText);
		} else {
			extt.log(Status.FAIL, elementName + "Text validate Failed. actualText -" + actualText + " && expectedtext -"
					+ expectedText);
	...
	/**
	 * @discription:this method is used to verify the actualText and ExpectedText to
	 *                   given attribute value
	 * @param we                     this parameter is used to search the element by
	 *                               locatorType and locatorValue on UI which type
	 *                               are webElement
	 * @param elementName            this parameter is used to we have to give
	 *                               elementName Which are we working on Element
	 *                               that time
	 * @param attributeValue         this parameter is used to verify the given any
	 *                               attribute value to match actual value
	...
	public void validateAttributeValue(WebElement we, String elementName, String attributeValue,
		...
		if (actualAttributeValue.equals(expectedAttributrValue)) {
			extt.log(Status.PASS, elementName + "Attribute validate Pass. actualAttributeValue -" + actualAttributeValue
					+ " && expectedAttributrValue -" + expectedAttributrValue);
		} else {
			extt.log(Status.FAIL, elementName + " Attribute validate Failed. actualAttributeValue -"
					+ actualAttributeValue + " && expectedAttributrValue -" + expectedAttributrValue);
...
//		WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
	...
	/**
	 * @discription: this method is used to check and verify weather check box is
	 *               selected or not
	 * @param we       this parameter is used to search the element by locatorType
	 *                 and locatorValue on UI which type are webElement
	 * @param checkBox this parameter is used to we have to give checkBoxName Which
	 *                 are we working
	 */
...
//		WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
	 ...
	 *                    webElement
	 * @param elementName this parameter is used to we have to give ElementName
	 *                    Which are we working on that time
	 * @return this method return Dimention class Object
	 */
	public Dimension sizeOfAnyElement(WebElement we, String elementName, int height, int width) {
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (chechElement == true) {
				...
				height = sizeOfElement.getHeight();
				width = sizeOfElement.getWidth();
				System.out.println("width.." + width + "height.." + height);
				extt.log(Status.INFO, MarkupHelper
						.createLabel(elementName + " width ..." + width + " height..." + height, ExtentColor.BROWN));
			} else {
				extt.log(Status.INFO,
						MarkupHelper.createLabel(elementName + "Element text size wrong", ExtentColor.BROWN));

			}
		} catch (NoSuchElementException e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + "Element text size not found", ExtentColor.BROWN));
			SnapShot("TakeScreenShotMethodOfSizeOfAnyElement");
			e.printStackTrace();
		} catch (Exception e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + "Element text size not found", ExtentColor.BROWN));
	...
	public void validateSizeOfAnyElement(WebElement we, String elementName, int width, int height) {
		try {
			...
			Dimension actualSize = sizeOfAnyElement(we, elementName, height, width);
			if (actualSize.equals(expectedSize)) {
				extt.log(Status.PASS,
						MarkupHelper.createLabel(elementName + " Size of element Validate Pass. ActualSize-- "
								+ actualSize + " && " + " ExpectedSize " + expectedSize, ExtentColor.GREY));
			} else {
				extt.log(Status.FAIL,
						MarkupHelper.createLabel(elementName + " Size of element Validate failed. ActualSize-- "
								+ actualSize + " && " + " ExpectedSize " + expectedSize, ExtentColor.GREY));
			}
		} catch (NoSuchElementException e) {
			extt.log(Status.FAIL,
					MarkupHelper.createLabel(elementName + " Size of element Validate failed..", ExtentColor.GREY));
			SnapShot("TAkeScreenShotMethodOfValidateSizeOfAnyElement");
			e.printStackTrace();
		} catch (Exception e) {
			extt.log(Status.FAIL,
					MarkupHelper.createLabel(elementName + " Size of element Validate failed..", ExtentColor.GREY));
	 ...
	 */
	public Point VerifylocationOfElement(WebElement we, String elementName, int xLocation, int yLocation) {
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (chekElement == true) {
				...
				xLocation = elementLocation.getX();
				yLocation = elementLocation.getY();
				extt.log(Status.INFO, MarkupHelper.createLabel(
						elementName + " xLocation ..." + xLocation + " yLocation..." + yLocation, ExtentColor.BROWN));
			} else {
				extt.log(Status.INFO,
						MarkupHelper.createLabel(elementName + " text location wrong", ExtentColor.BROWN));
			}
		} catch (NoSuchElementException e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + " text location not found", ExtentColor.BROWN));
			SnapShot(elementName);
			e.printStackTrace();
		} catch (Exception e) {
			extt.log(Status.INFO,
	...
	public void validateLocationOfElement(WebElement we, String elementName, int x, int y) {
		...
		Point actualLocation = VerifylocationOfElement(we, elementName, x, y);
		if (actualLocation.equals(expectedLocation)) {
			extt.log(Status.PASS,
					MarkupHelper.createLabel(elementName + " Location of element Validate Pass. actualLocation-- "
							+ actualLocation + " && " + " expectedLocation " + expectedLocation, ExtentColor.GREY));
		} else {
			extt.log(Status.FAIL,
					MarkupHelper.createLabel(elementName + " Location of element Validate Failed. actualLocation-- "
							+ actualLocation + " && " + " expectedLocation " + expectedLocation, ExtentColor.GREY));
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (check == true) {
				...
				extt.log(Status.INFO, MarkupHelper
						.createLabel(elementName + " color of element is " + actualColorOfElement, ExtentColor.GREEN));
			}
		} catch (NoSuchElementException e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + " color of element isn't found ", ExtentColor.GREEN));

			SnapShot("TakeScreenShotMethodOfVerifyElementOfColor");
			e.printStackTrace();
		} catch (Exception e) {
			extt.log(Status.INFO,
					MarkupHelper.createLabel(elementName + " color of element isn't found ", ExtentColor.GREEN));
	 ...
	 *                          webElement
	 * @param elementName       this parameter is used to we have to give
	 *                          elementName Which are we working on that time
	 * @param checkElementColor this parameter is used to we have to give color as a
	 *                          String
	 * @param expectedColor
	 */
	public void ValidatElementOfColor(WebElement we, String elementName, String ColorName, String expectedColor) {
		String actualColor = verifyElementOfColor(we, elementName, ColorName);
		if (actualColor.equalsIgnoreCase(expectedColor)) {
			extt.log(Status.PASS,
					MarkupHelper.createLabel(elementName + " Color Of Element Validate pass. actualColor--"
							+ actualColor + "&&" + "ExpectedColor--" + expectedColor, ExtentColor.PURPLE));
		} else {
			extt.log(Status.FAIL,
					MarkupHelper.createLabel(elementName + " Color Of Element Validate failed. actualColor--"
							+ actualColor + "&&" + "ExpectedColor--" + expectedColor, ExtentColor.PURPLE));
	 ...
	 * @param elementName     this parameter is used to we have to give elementName
	 *                        Which are we working on that time
	 * @param backgroundColor this parameter is used to we have to give backgorund
	 *                        as a String type
	 * @return
	 */
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
			...
			if (checkElementVisibility == true) {
				...
				extt.log(Status.INFO, MarkupHelper.createLabel(
						elementName + " color of element is " + actualBackgroundColorOf, ExtentColor.GREEN));
	...
	/**
	 * @discription: this parameter is used to validate backgound color of the
	 *               element
	 * @param we              this parameter is used to search the element by
	 *                        locatorType and locatorValue on UI which type are
	 *                        webElement
	 * @param elementName     this parameter is used to we have to give elementName
	 *                        Which are we working on that time
	 * @param backgroundColor this parameter is used towe have to give backgorund as
	 *                        a String type
	 * @param
	 */
	public void validateBackgroundColorOfElement(WebElement we, String elementName, String backgroundColor,
		...
		if (actualBackground.equals(expextedBackgoundColor)) {
			extt.log(Status.PASS,
					MarkupHelper
							.createLabel(
									elementName + "Backgound Color Of Element Validate pass. actualColor--"
											+ actualBackground + "&&" + "ExpectedColor--" + expextedBackgoundColor,
									ExtentColor.PURPLE));
	...
	public void getWindowHandles(String expectedTitle) {
		...
		for (String getHandleWindow : getHandleValue) {
			...
			extt.log(Status.INFO,
					MarkupHelper.createLabel(expectedTitle + " is Title switch successfully", ExtentColor.BLUE));
	 ...
	 * @param expectedCurrentUrl this parameter is used to give current url of the
	 *                           page
	 */
	public void validateGetCurrentUrl(String expectedCurrentUrl) {
		...
		for (String getHandleWindow : getHandleValue) {
			...
			if (actualWindow.equalsIgnoreCase(expectedCurrentUrl)) {
				extt.log(Status.INFO, MarkupHelper
						.createLabel(expectedCurrentUrl + " is CurrentUrl switch successfully", ExtentColor.BLUE));
				break;
			}
	...
	/**
	 * this parameter is used to whether the select element supports multiple
	 * selection options at the same time or not
	 * 
	 * @param we          this parameter is used to search the element by
	 *                    locatorType and locatorValue on UI which type are
	 *                    webElement
...
//			WebElement we=getWebElementLocatorXpath(locatorValue, locatorType);
				...
				if (multipleSelect == true) {
					extt.log(Status.PASS,
							MarkupHelper.createLabel(
									elementName + "is multiple Selected option validate Pass" + multipleSelect,
									ExtentColor.PINK));
				} else {
					extt.log(Status.FAIL,
							MarkupHelper.createLabel(
									elementName + "is multiple Selected option validate failed" + multipleSelect,
									ExtentColor.PINK));
				}
			}
		} catch (NoSuchElementException e) {
	...
	public String getTitleofpage(String title) {
